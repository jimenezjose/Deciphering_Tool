...
int main() {
  ...
  for( chapter = 0; chapter < size; chapter++ ) {
    ...
    fprintf(stdout, "number of Characters: %ld\n", (numOfChars - numOfSpaces) );
    fprintf(stdout, "number of Words: %ld\n", numOfWords);
...
long shiftCipher( long shift, const char * cipher ) {
  ...
  fprintf(stdout, "Possible Ceasar Cipher.\n"); 
  fprintf(stdout, "Shifting Cipher by %ld\n", shift);
...
long askUser(const char * when) {
  ...
  do {
    ...
    fprintf(stdout, "Press Enter to %s: ", when);
